...
You can find the detailed tutorial on <a href="https://docs.djangoproject.com/en/3.1/intro/tutorial01/" target="_blank" rel="noopener noreferrer nofollow">official django website</a>.
...
We have prepared a <a href="https://github.com/SigNoz/sample-django" target="_blank" rel="noopener noreferrer nofollow">sample Django application</a> to speed up the setup process. We'll be using the Django admin panel to create and interact with polls.
...
We're almost done. In the last step, we just need to configure a few environment variables for the <a href="https://opentelemetry.io/docs/specs/otel/protocol/exporter/" target="_blank" rel="noopener noreferrer nofollow">OTLP exporters</a>.
...
Use OpenTelemetry Python configuration to exclude URLs. OpenTelemetry provides <a href="https://opentelemetry.io/docs/zero-code/python/configuration/#excluded-urls" target="_blank" rel="noopener noreferrer nofollow">Python-specific environment variables</a> for exclusions. You can set a global excluded URL pattern (`OTEL_PYTHON_EXCLUDED_URLS`) list, or set Django-specific excluded URL patterns (`OTEL_PYTHON_DJANGO_EXCLUDED_URLS`). This is a common best practice to reduce noise and cost while keeping high-value traces.
