...
You can access the running app at <a href="http://localhost:8090" target="_blank" rel="noopener noreferrer nofollow">localhost:8090</a>
...
From the above image, no GET request was returned. This is because the Java application hasn’t been accessed at <a href="http://localhost:8090" target="_blank" rel="noopener noreferrer nofollow">localhost:8090</a> yet. To generate an HTTP GET request, visit the application at <a href="http://localhost:8090" target="_blank" rel="noopener noreferrer nofollow">localhost:8090</a> and try to refresh it one or two times. After that, on your SigNoz dashboard, still on the Traces Explorer page, change the duration to the ‘Last 5 min’, give it some time, and then run the same query again. You should see the GET requests you generated.
