...
- The <a href="https://opentelemetry.io/docs/specs/otel/logs/bridge-api/" target="_blank" rel="noopener noreferrer nofollow">Bridge API</a> is intended to connect existing logging with OpenTelemetry. It can be used together with existing logging libraries to automatically inject the trace context in the emitted logs, and provide an easy way to send the logs via OTLP.
- File or stdout export - if you have access to read your logging file periodically or tail your logs, you can parse bare files or .csv or json-formatted and send them to the collector. If you cannot parse your logs yourself, another log collection agent, such as [FluentBit](https://signoz.io/docs/userguide/fluentbit_to_signoz/) can collect the logs, <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/tree/master/receiver/fluentforwardreceiver" target="_blank" rel="noopener noreferrer nofollow">then send the logs</a> to OpenTelemetry Collector.
...
Incoming logs are parsed here by the `regex_parser` operator which examines incoming strings and sets attributes. This is documented within the <a href="https://github.com/open-telemetry/opentelemetry-collector-contrib/blob/main/pkg/stanza/docs/operators/regex_parser.md" target="_blank" rel="noopener noreferrer nofollow">opentelemetry-collector-contrib</a> repo. Note that altering the tcplog/docker receiver may result in missing data. The operator makes use of <a href="https://github.com/google/re2/wiki/Syntax" target="_blank" rel="noopener noreferrer nofollow">Go regular expressions</a>. When writing a regex, consider using a tool such as <a href="https://regex101.com/?flavor=golang" target="_blank" rel="noopener noreferrer nofollow">regex101</a>.
