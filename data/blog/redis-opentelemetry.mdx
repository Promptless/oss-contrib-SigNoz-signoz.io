...
Once installed, you can access SigNoz UI at port 3301 -  <a href="http://localhost:3301/application" target="_blank" rel="noopener noreferrer nofollow">http://localhost:3301</a>. You will get a sign-up page. If you’re a first-time user, you can create an account using `Create an account`.
...
You can have a look at the `otel-collector-config.yaml` file on our <a href="https://github.com/SigNoz/signoz/blob/main/deploy/docker/otel-collector-config.yaml" target="_blank" rel="noopener noreferrer nofollow">GitHub repo</a>. The final config file along with Redis receiver looks like below:
