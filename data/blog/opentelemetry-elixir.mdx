...
As it is documented in the `opentelemetry_phoenix` and `opentelemetry_ecto` <a href="http://hexdocs.pm" target="_blank" rel="noopener noreferrer nofollow">hexdocs.pm</a> pages, we need to initialize both telemetry handlers.
...
In this <a href="https://github.com/SigNoz/elixir_otel_sample" target="_blank" rel="noopener noreferrer nofollow">GitHub repository</a> you’ll find an Elixir Phoenix API that exposes a couple of endpoints but we’ll focus on one:  `/api/users` that will return a list of users that are stored on the respective database table.
...
You can access SigNoz UI at <a href="http://localhost:3301/application" target="_blank" rel="noopener noreferrer nofollow">http://localhost:3301/application</a> (after signing up) to see your Elixir Phoenix API being monitored. The other applications that you see are sample applications that come bundled with the installation.
...
You can go to <a href="http://localhost:3301/errors" target="_blank" rel="noopener noreferrer nofollow">http://localhost:3301/errors</a> to check out the errors in your application.
