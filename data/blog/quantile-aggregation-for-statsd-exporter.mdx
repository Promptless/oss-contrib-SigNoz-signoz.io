...
Let's try to implement this into our existing project. Checkout branch `quantile_aggregation` from our <a href="https://github.com/ankitnayan/django_sample_project" target="_blank" rel="noopener noreferrer nofollow">**github repo**</a>. This contains `config/statsd-mapping.conf` file which includes:
...
To get the 5 slowest endpoints follow blog <a href="https://www.robustperception.io/graph-top-n-time-series-in-grafana" target="_blank" rel="noopener noreferrer nofollow">Graph top N time series in Grafana</a>. A slight change will be the Regex expression for the variable. Check mine from the below image.
