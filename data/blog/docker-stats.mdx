...
2.  `--format` which uses the <a href="https://pkg.go.dev/text/template" target="_blank" rel="noopener noreferrer nofollow">Go Template syntax</a> to print images out.
...
It uses <a href="https://opentelemetry.io/" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry</a> to collect metrics from your containers for monitoring. OpenTelemetry is becoming the world standard for instrumentation of cloud-native applications, and it is backed by <a href="https://www.cncf.io/" target="_blank" rel="noopener noreferrer nofollow">CNCF</a> foundation, the same foundation under which Kubernetes graduated.
...
The install script automatically installs Docker Engine on Linux. However, you must manually install <a href="https://docs.docker.com/engine/install/" target="_blank" rel="noopener noreferrer nofollow">Docker Engine</a> on macOS before running the install script.
