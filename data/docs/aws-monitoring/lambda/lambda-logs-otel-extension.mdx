...
```
Lambda function
  └── Extension Layer (OTel Collector)
        ├── telemetryapi receiver  ← subscribes to Lambda Telemetry API
        ├── batch processor
        ├── decouple processor     ← decouples export from execution path
        └── otlphttp exporter      → SigNoz
```
