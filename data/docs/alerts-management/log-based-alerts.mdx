...
description: Create alerts based on log data using query conditions, pattern matching, and log level filters.
---

A Log-based alert allows you to define conditions based on log data, triggering alerts when these conditions are met.

## Creating a Log-based Alert

When you click **"New Alert"** in SigNoz, you'll first see an alert type selection page with 5 options. Select **"Logs-based Alert"** to proceed with creating a log-based alert.
...
- **Logs tab** (default for log-based alerts): Use the Logs Query Builder to define your log filters and aggregations
- **Query Builder**: Visual interface to filter logs and define conditions
- **ClickHouse Query**: Write raw SQL queries for logs
...
## Step 1: Define the Log Metric (Classic UI)
