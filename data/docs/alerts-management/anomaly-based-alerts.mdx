...
An Anomaly-based alert in SigNoz allows you to define conditions based on metric data and trigger alerts when the metric data deviates from the expected behavior.

## Creating an Anomaly-based Alert

When you click **"New Alert"** in SigNoz, you'll first see an alert type selection page with 5 options. Select **"Anomaly based Alert"** (marked as Beta) to proceed with creating an anomaly-based alert.
...
- **Metrics tab** (default for anomaly-based alerts): Use the Query Builder to select your metric
...
Define when your alert should trigger based on anomaly detection:

- **Seasonality**: Choose the seasonality pattern (Hourly, Daily, Weekly)
- **Z-score Threshold**: Set the sensitivity threshold for anomaly detection
- **Condition**: Specify when to trigger (Above, Below, Above or Below)
- **Occurrence**: Choose how many times the condition must match (At least once, Every time)

#### Advanced Options

- **Alert Frequency**: How often SigNoz checks for anomalies
