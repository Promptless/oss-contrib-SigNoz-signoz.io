...
description: Create alerts based on trace data including latency, span count, error rates, and other trace attributes.
---

A Trace-based alert in SigNoz allows you to define conditions based on trace data, triggering alerts when these conditions are met.

## Creating a Trace-based Alert

When you click **"New Alert"** in SigNoz, you'll first see an alert type selection page with 5 options. Select **"Trace-based Alert"** to proceed with creating a trace-based alert.
...
- **Traces tab** (default for trace-based alerts): Use the Traces Query Builder to define your trace filters and aggregations
- **Query Builder**: Visual interface to filter traces and define conditions
- **ClickHouse Query**: Write raw SQL queries for trace data

### Step 3: Configure Alert Conditions and Thresholds

Define when your alert should trigger using the same condition options as metrics:
...
## Step 1: Define the Trace Metric (Classic UI)
