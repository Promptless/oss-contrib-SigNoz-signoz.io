...
This guide covers the **direct OpenTelemetry instrumentation** method using the OTel Lambda Layer. If you prefer to collect logs via CloudWatch Logs and S3, please refer to the [AWS Lambda Logs via S3](https://signoz.io/docs/aws-monitoring/lambda/lambda-logs/) guide.
</Admonition>
...
This method captures everything written to `stdout` and `stderr` (e.g., `console.log`) as well as AWS Platform logs (`REPORT`, `START`, `END`). It requires zero code changes and works by running an OTel Collector as a Lambda extension layer that subscribes to Lambda's Telemetry API.

See the dedicated guide for full setup steps:

**[Send AWS Lambda logs to SigNoz using the OTel Collector Extension Layer](https://signoz.io/docs/aws-monitoring/lambda/lambda-logs-otel-extension/)**
