...
description: Learn how to send logs from AWS Lambda Node.js functions to SigNoz using OpenTelemetry auto-instrumentation.
...
**Want zero-code log collection instead?** If you want to capture all `console.log`, `stdout`/`stderr`, and AWS Platform logs (`START`, `END`, `REPORT`) without any code changes, use the [OTel Collector Extension Layer guide](https://signoz.io/docs/aws-monitoring/lambda/lambda-logs-otel-extension/) instead.

If you prefer routing logs via CloudWatch Logs and S3, see the [AWS Lambda Logs via S3](https://signoz.io/docs/aws-monitoring/lambda/lambda-logs/) guide.
</Admonition>
