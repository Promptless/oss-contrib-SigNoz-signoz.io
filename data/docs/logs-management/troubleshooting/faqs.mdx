...
### Q. How many log records can I download at once?
A. You can download up to 50,000 log records directly from the UI. For larger datasets, use the SigNoz API.

<details>
<summary>Show detailed steps</summary>

You can download log records directly from the SigNoz UI with the following options:
- 10,000 rows
- 30,000 rows
- 50,000 rows

To download a larger dataset beyond 50,000 logs, you can use the SigNoz API instead:

1. Open your browser's DevTools → Network tab
2. Perform a log search in the SigNoz UI
3. In the Network tab, copy the `query_range` request whose `request_type` is `raw` (if multiple `query_range` requests are available, choose the one with `request_type=raw`)
4. Increase the `limit` value in the request to retrieve more records (see [Pagination in Log Search](https://signoz.io/docs/logs-management/logs-api/search-logs/#pagination-in-log-search))
5. Run the command to get the data in JSON format

For detailed instructions on using the API for log downloads, refer to the [Search Logs API documentation](https://signoz.io/docs/logs-management/logs-api/search-logs/).
