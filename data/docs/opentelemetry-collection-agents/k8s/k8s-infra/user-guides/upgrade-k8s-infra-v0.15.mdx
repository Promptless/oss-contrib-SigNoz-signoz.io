...
**Telemetry Metrics Address**: The `service::telemetry::metrics::address` field is no longer supported. <a href="https://github.com/open-telemetry/opentelemetry-collector/releases/tag/v0.123.0" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry Collector v0.123.0 deprecated this field</a>, and it's now completely ignored. The collector manages telemetry metrics exposure internally, so you should remove any references to:
...
The OpenTelemetry Collector exposes its own internal metrics (such as dropped spans and queue size) for Prometheus to scrape by default at <a href="http://127.0.0.1:8888/metrics" target="_blank" rel="noopener noreferrer nofollow">http://127.0.0.1:8888/metrics</a>. If you want to access these metrics outside the pod, configure the telemetry reader to bind to `0.0.0.0:8888` instead of the default.

For more details, see the <a href="https://opentelemetry.io/docs/concepts/collector/internal-telemetry/" target="_blank" rel="noopener noreferrer nofollow">OpenTelemetry Collector Internal Telemetry documentation</a>.
