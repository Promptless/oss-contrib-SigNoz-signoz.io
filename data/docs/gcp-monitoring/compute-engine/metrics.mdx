...
Under the `googlecloudmonitoring` receiver, you need to specify the metrics that you want to capture in the `metrics_list`. You can include all the metrics that you would like to capture for Compute Engine, see the <a href="https://cloud.google.com/monitoring/api/metrics_gcp#gcp-compute" target="_blank" rel="noopener noreferrer nofollow">available metrics for Compute Engine</a>.
