...
- `<region>`: your SigNoz Cloud region (`us`, `eu`, or `in`). See [SigNoz Cloud endpoints](https://signoz.io/docs/ingestion/signoz-cloud/overview/#endpoint)
...
OpenTelemetry JS recommends using matching `latest` package lines together. See the [OpenTelemetry JS compatibility matrix](https://github.com/open-telemetry/opentelemetry-js#package-version-compatibility).
</details>
...
Set `OTEL_SERVICE_NAME` to a stable value, redeploy, and invoke again. If unset, the fallback name is derived from the function ID (for example, `function-a`).
