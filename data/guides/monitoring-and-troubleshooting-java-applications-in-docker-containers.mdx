...
In that case we are not required to locate `prometheus.yml` and modify it for default variables like <a href="http://localhost" target="_blank" rel="noopener noreferrer nofollow">localhost</a> or scrape interval. These can be easily modified in the Docker UI when we run its container. 
