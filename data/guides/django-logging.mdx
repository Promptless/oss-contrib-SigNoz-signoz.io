...
If you already have a Django app running, skip ahead to the [How Django Logging Works](#how-to-use-logging-in-django) section. Otherwise, follow the steps below to create a fresh project that we will use throughout this guide.
...
If you want to go further, here are some useful next steps:

- Set up [Alerts in SigNoz](/docs/alerts/) to be notified when the ERROR log volume exceeds a threshold.
- Add custom traces and metrics alongside logs using the [OpenTelemetry Django Instrumentation guide](/blog/opentelemetry-django/).
- Explore [Python logging best practices](/guides/python-logging-best-practices/), including per-module loggers, exception handling, and avoiding common pitfalls.
